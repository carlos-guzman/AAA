...
class Scraper:
  ...
  def get_all_sections(self):
    ...
    with open('{}college-classifications'.format(self.DUMP_DIR), 'w') as f:
      for course in data:
  ...
  def summarize_detail(self, id, soup):
    ...
    for table in tables:
      ...
      classnum, session, section, is_open, grading, loccode, component, notes = [None]*8
      ...
      for element in meet_data_arr:
        time_re = re.search('\d+\.\d+ ?[AP]M[ -]*\d+\.\d+ ?[AP]M', element[24:]) 
        if time_re:
          time_arr = time_re.group(0).split(' - ')
        days_re = re.search('\w{3}(,\w{3})*', element[24:])
        if days_re:
          days_arr = days_re.group(0).split(',')
        for day in days_arr:
